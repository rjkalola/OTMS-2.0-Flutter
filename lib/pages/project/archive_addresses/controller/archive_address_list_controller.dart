...
import 'package:otm_inventory/pages/project/archive_addresses/controller/archive_address_list_repository.dart';
import 'package:otm_inventory/pages/project/archive_projects/controller/archive_project_list_repository.dart';
...
class ArchiveAddressListController extends GetxController
    implements MenuItemListener, DialogButtonClickListener {
  final _api = ArchiveAddressListRepository();
  ...
  int selectedIndex = 0, selectedAddressId = 0,selectedProjectId = 0;
  ...
  void onInit() {
    ...
    if (arguments != null) {
      selectedProjectId = arguments[AppConstants.intentKey.projectId];
    }
    getArchiveAddressListApi();
  }
  Future<void> getArchiveAddressListApi() async {
    isLoading.value = true;
    Map<String, dynamic> map = {};
    map["project_id"] = selectedProjectId;
    _api.archiveAddressList(
      queryParameters: map,
  ...
  void unArchiveAddressApi(int id, int index) {
    isLoading.value = true;
    Map<String, dynamic> map = {};
    // map["company_id"] = ApiConstants.companyId;
    map["id"] = id;
    _api.unArchiveAddress(
      data: map,
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.isSuccess) {
  ...
  void deleteAddressApi(int id, int index) {
    isLoading.value = true;
    Map<String, dynamic> map = {};
    map["id"] = id;
    _api.deleteAddress(
  ...
  showDeleteProjectDialog(int id, int index) async {
  ...
  void onPositiveButtonClicked(String dialogIdentifier) {
    if (dialogIdentifier == AppConstants.dialogIdentifier.deleteProject) {
      deleteAddressApi(selectedAddressId, selectedIndex);
      Get.back();
    }
  }
