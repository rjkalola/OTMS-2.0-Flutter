...
import 'package:belcka/pages/users/archive_user_list/controller/archive_user_list_repository.dart';
...
class ArchiveUserListController extends GetxController
    implements MenuItemListener, DialogButtonClickListener {
  final _api = ArchiveUserListRepository();
  ...
  int selectedIndex = 0, selectedUserId = 0;
  ...
  void unArchiveUserApi(int userId, int index) {
    isLoading.value = true;
    Map<String, dynamic> map = {};
    map["company_id"] = ApiConstants.companyId;
    map["user_ids"] = userId.toString();
    _api.unarchiveUserAccount(
      data: map,
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.isSuccess) {
  ...
  showUnArchiveUserDialog(int userId, int index) async {
    selectedUserId = userId;
    selectedIndex = index;
    AlertDialogHelper.showAlertDialog(
        "",
        'are_you_sure_you_want_to_unarchive'.tr,
        'yes'.tr,
        'no'.tr,
        "",
        true,
        false,
        this,
        AppConstants.dialogIdentifier.unArchiveUser);
  }
  ...
  void onPositiveButtonClicked(String dialogIdentifier) {
    if (dialogIdentifier == AppConstants.dialogIdentifier.unArchiveUser) {
      unArchiveUserApi(selectedUserId, selectedIndex);
      Get.back();
    }
  }
