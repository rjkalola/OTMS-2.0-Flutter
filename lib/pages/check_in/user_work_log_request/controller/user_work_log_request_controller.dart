...
import 'package:belcka/pages/check_in/user_work_log_request/controller/user_work_log_request_repository.dart';
import 'package:belcka/pages/check_in/user_work_log_request/model/user_work_log_details_info.dart';
import 'package:belcka/pages/check_in/user_work_log_request/model/user_work_log_request_details_response.dart';
...
class UserWorkLogRequestController extends GetxController
  ...
  final _api = UserWorkLogRequestRepository();
  ...
  final workLogInfo = UserWorkLogDetailsInfo().obs;
  int requestLogId = 0;
  ...
  bool get isPending =>
      (workLogInfo.value.status ?? 0) == AppConstants.status.pending;

  bool get showAdminFooter =>
      isPending &&
      UserUtils.isAdmin() &&
      !UserUtils.isLoginUser(workLogInfo.value.userId);
  ...
  String get formattedWorkedTime =>
      DateUtil.seconds_To_HH_MM(workLogInfo.value.payableWorkSeconds ?? 0);

  int get requestStatus => workLogInfo.value.status ?? 0;

  bool get isApproved => requestStatus == AppConstants.status.approved;

  bool get isRejected => requestStatus == AppConstants.status.rejected;

  String get hoursTitle {
    if (isApproved) return '${'approved_hours'.tr}:';
    if (isRejected) return '${'rejected_hours'.tr}:';
    return 'work_time'.tr;
  }

  String get noteLabel {
    if (isApproved) return 'approved_note'.tr;
    if (isRejected) return 'rejected_note'.tr;
    return 'requested_note'.tr;
  }

  String? get displayNote {
    if (isRejected) {
      return !StringHelper.isEmptyString(workLogInfo.value.rejectReason)
          ? workLogInfo.value.rejectReason
          : workLogInfo.value.note;
    }
    return workLogInfo.value.note;
  }

  String get statusLabel =>
      StringHelper.capitalizeFirstLetter(workLogInfo.value.statusText ?? "");

  void onApproveTap() {
    if (valid()) {
      showActionDialog(AppConstants.dialogIdentifier.approve);
    }
  }

  void onRejectTap() {
    if (valid()) {
      showActionDialog(AppConstants.dialogIdentifier.reject);
    }
  }
    ...
    _api.getWorkLogRequestDetails(
      ...
      onSuccess: (ResponseModel responseModel) {
        if (responseModel.isSuccess) {
          ...
          UserWorkLogRequestDetailsResponse response =
              UserWorkLogRequestDetailsResponse.fromJson(
